...
function addToCart(name, price, link, variant) {
    ...
    if(existing) {
        ...
        cart.push({ name: name, price: price, quantity: 1, link: link, variant: variant });
    }
...
function buyNow(name, price, variant) {
    let confirmCheckout = confirm(`Are you sure you want to purchase ${name} ${variant} for ₱${price}`);
    if (confirmCheckout) {
